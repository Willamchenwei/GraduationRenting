			...
			<div class="head_logo2">
				...
				<ul class="rightoflogo2">
					<li><a href="" onmouseover="mouseOver2()"
						onmouseout="mouseOut2()"><%=user.getUserName() %> <img id="pull2"
				...
				<ul>
					<li id="s_01" class="" onmouseover="mouseOverul1()"
						onmouseout="mouseOutul1()"><a href="order.jsp"><img
							id="order1" src="resource/images/order1b.png"><span></span>订单</a></li>
					<li id="s_02" class="" onmouseover="mouseOverul2()"
						onmouseout="mouseOutul2()"><a href="my_message.jsp"><img
							id="order2" src="resource/images/order2b.png"><span></span>我的消息</a></li>
					<li id="s_03" class="cur" onmouseover="mouseOverul3()"><a
						href="personal_information.html"><img id="order3"
					...
					<div class="data_box pa20" id="identityInfo">
						...
						<ul class="data_norms">
							<li><label class="w74">用户名</label> <input
								...
								value=<%=user.getUserName() %> style="display: inline-block;" placeholder="请输入用户名">
							</li>
							<li><label class="w74">密码：</label> <input
								class="data_norms_input" type="password" id="password" name = "password" value=<%=user.getPassword() %>
							...
							<li><label class="w74">真实姓名：</label> <input
								...
								<%if (user.getRealName() != null) { %> value=<%=user.getRealName() %> <%} %>style="display: inline-block;" placeholder="请输入真实姓名">
							</li>
							<li><label class="w74">身份证：</label> <input
								class="data_norms_input" type="text" id="idCard" name = "idCard" <%if (user.getIdCard() != null) { %> value=<%=user.getIdCard()%><%} %>
							...
							<li><label class="w74">地址：</label> <input
								...
								id="address" name = "address" <%if (user.getAddress() != null) { %>value=<%=user.getAddress() %> <%} %>style="display: inline-block;"></li>
							<li><label class="w74">电话：</label> <input
								class="data_norms_input" type="text" id="phone" name = "phone" <%if (user.getPhone() != null) {%>value=<%=user.getPhone() %><%} %>
							...
							<li><label class="w74">邮箱：</label> <input
								...
								id="emil" name = "emil" <%if (user.getEmil()!= null) { %>value=<%=user.getEmil() %> <%} %>style="display: inline-block;"></li>
						</ul>
					</div>
				</div>
		</div>
